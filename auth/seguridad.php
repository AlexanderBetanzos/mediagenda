...
?>
<h1 class="h3 mb-1"><i class="bi bi-shield-lock text-brand"></i> Seguridad de la cuenta</h1>
<p class="text-muted">Protege tu acceso con verificación en dos pasos (2FA).</p>

<div class="row">
  <div class="col-lg-7">
    <div class="card">
      <div class="card-header bg-white fw-semibold d-flex justify-content-between align-items-center">
        <span><i class="bi bi-phone"></i> Verificación en dos pasos</span>
        <?php if ($tiene2fa): ?>
      ...
      <div class="card-body">
      <?php if ($tiene2fa): ?>
        <p>Tu cuenta pide un código de tu app de autenticación al iniciar sesión.</p>
        <form method="post" class="row g-2 align-items-end" onsubmit="return confirm('¿Desactivar la verificación en dos pasos?');">
            <?= csrf_field() ?>
            <input type="hidden" name="accion" value="desactivar">
            <div class="col-sm-7">
                <label class="form-label">Confirma tu contraseña para desactivar</label>
                <input type="password" name="password" class="form-control" required>
            </div>
            <div class="col-sm-5">
                <button class="btn btn-outline-danger"><i class="bi bi-shield-slash"></i> Desactivar 2FA</button>
            </div>
        </form>

      <?php elseif (!$setup): ?>
        <p class="text-muted">Usa Google Authenticator, Authy o Microsoft Authenticator. Es gratis y funciona sin internet.</p>
        <form method="post">
            <?= csrf_field() ?>
            <input type="hidden" name="accion" value="iniciar">
            <button class="btn btn-primary"><i class="bi bi-shield-plus"></i> Activar 2FA</button>
        </form>

      <?php else: ?>
        <ol class="mb-3">
            <li>Abre tu app de autenticación y escanea este código QR.</li>
            <li>Escribe el código de 6 dígitos que aparezca para confirmar.</li>
        </ol>
        <div class="text-center mb-3">
            <div id="qr" class="d-inline-block p-2 bg-white border rounded"></div>
            <div class="small text-muted mt-2">¿No puedes escanear? Clave manual:
        ...
        <form method="post" class="row g-2 justify-content-center">
            ...
            <div class="col-auto">
                <button class="btn btn-primary"><i class="bi bi-check2-circle"></i> Confirmar y activar</button>
            </div>
        </form>
