    ...
    <div class="card-body p-4" style="color:#1f2d3d;background:#fff">
        <div class="d-flex justify-content-between border-bottom pb-3 mb-3">
            ...
            <div class="text-end">
                ...
                <div><strong>Fecha:</strong> <?= fmt_fecha($f['fecha']) ?></div>
                <span class="badge bg-<?= $badge ?>"><?= ucfirst($f['estado']) ?></span>
            </div>
        </div>

        <div class="mb-3">
            <strong>Cliente:</strong> <?= e($f['pac_nombre'].' '.$f['pac_ape']) ?><br>
        ...
        <table class="table table-bordered">
            <thead><tr><th>Descripción</th><th class="text-center">Cant.</th><th class="text-end">Precio</th><th class="text-end">Importe</th></tr></thead>
            <tbody>
            ...
            <tfoot>
                <tr><td colspan="3" class="text-end">Subtotal</td><td class="text-end"><?= fmt_money($f['subtotal']) ?></td></tr>
                <?php if ($f['descuento'] > 0): ?>
                <tr><td colspan="3" class="text-end">Descuento</td><td class="text-end">-<?= fmt_money($f['descuento']) ?></td></tr>
                <?php endif; ?>
                <tr class="fw-bold"><td colspan="3" class="text-end">Total (<?= e(moneda()) ?>)</td><td class="text-end" style="color:#0b6fb8"><?= fmt_money($f['total']) ?></td></tr>
            </tfoot>
        </table>

        <?php if ($f['metodo_pago']): ?><p><strong>Método de pago:</strong> <?= e($f['metodo_pago']) ?></p><?php endif; ?>
