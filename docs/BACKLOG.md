...
| Expediente inteligente | ✅ Identificación (CURP/RFC/INE/tipo sangre), contacto de emergencia, antecedentes (personales/familiares/cirugías/vacunas/crónicas/hábitos/alergias), **IMC automático**, fotos/PDF/estudios ✅. ✅ **Signos vitales ampliados** (peso, estatura, presión, temperatura, **glucosa, FC, SpO2, FR**) + **gráficas de evolución** (Chart.js) en el expediente. ✅ **Medicamentos actuales estructurados** (pestaña, alta/suspensión, tabla `paciente_medicamentos`). ⬜ Falta: gráficas de percentiles pediátricos exactos | B | 3.2 NOM-024 | 🟢 |
| Consulta avanzada | ✅ **Plantillas de consulta** por **especialidad** (formatos reutilizables que pre-llenan el expediente, agrupadas por especialidad, con paquete inicial de 14 formatos de muchas áreas, `plantillas/`), diagnósticos/tratamientos ✅. ⬜ Falta: notas rápidas, órdenes médicas, interconsultas, firma digital, consentimiento informado | B | Expediente | 🟢 |
...
| Módulo | Qué incluye | Plan | Depende de | Estado |
|---|---|---|---|---|
| Telemedicina | ✅ **Videoconsulta por cita**: la cita se marca como `en_linea` y recibe una sala propia. El médico entra desde el panel (`telemedicina/sala.php`) y el paciente desde un enlace público con el token de su cita (`telemedicina/consulta.php`), sin cuenta ni contraseña, igual que el flujo de confirmar. Ventana de acceso: abre 15 min antes y cierra una hora después de terminar, para que el enlace del recordatorio no abra salas muertas. El botón de entrar aparece solo en la página de confirmación que el paciente ya recibe por correo. Chat, compartir pantalla y silenciar/expulsar vienen de Jitsi embebido; el servidor sale de `cfg('jitsi_dominio')` y por omisión es la instancia pública, así que **no hay costo ni cuenta que abrir**. El nombre de la sala es un valor aleatorio distinto del token, para que el servidor de video nunca vea la credencial del paciente (`telemedicina/`). Gateado `require_modulo('telemedicina')` (Profesional y Clínica). ⬜ Falta: grabación de la sesión, sala de espera virtual con varios pacientes en fila | P (básica) / C (grabación) | Portal + agenda | 🟢 |
...
| Especialidades | ✅ **Odontograma por caras** (`odontograma/`, FDI, capas hallazgo/requerido/realizado, **historial** de versiones, **genera presupuesto** con lo requerido y marca la cara al ejecutarlo) · ✅ **Curvas de crecimiento** pediátricas (`crecimiento/index.php`) · ✅ **Ginecología/Obstetricia — control prenatal** (`prenatal/`: FUM→SDG/FPP, GPA, riesgo, visitas con peso/TA/FCF/altura uterina y gráficas) · ✅ **Nutrición** (`nutricion/`: antropometría, IMC con clasificación OMS, meta y plan kcal, evolución graficada) · ✅ **Cardiología** (`cardiologia/`: TA/FC, perfil de lípidos, factores de riesgo, NYHA, riesgo CV, ECG, evolución graficada) · ✅ **Dermatología** (`dermatologia/`: lesiones con seguimiento foto-comparativo) · ✅ **Psicología** (`psicologia/`: sesiones + escalas PHQ-9/GAD-7 con interpretación y evolución) · ✅ **Oftalmología clínica** (`oftalmologia/`: AV, PIO OD/OI con gráfica para glaucoma, segmento anterior, fondo de ojo; complementa Óptica). Las verticales se agrupan en el menú "Especialidad" del expediente. ⬜ Falta (opcional): periodontograma, percentiles pediátricos exactos | P (paquete) / Add-on | Consulta avanzada + plantillas | 🟢 |
...
| Módulo | Qué incluye | Plan | Depende de | Estado |
|---|---|---|---|---|
| Micrositio por consultorio + reserva y pago | ✅ Página pública por slug (`/c/<slug>`, estilo GymOS `?t=`): la raíz sigue siendo la landing del producto; con el slug se muestra la del consultorio (marca, servicios con precio, equipo, contacto, horario resumido de `medico_horarios`, mapa). Indexable por Google. ✅ La **reserva va embebida** en el micrositio (`#agendar`, lógica compartida `includes/agenda_reservar_logica.php`/`_render.php`, reusada también por `/agenda/reservar`). ✅ **Pago en línea de la cita**: si el consultorio pone precio (`agenda_online_precio`) y tiene Mercado Pago, el paciente paga al agendar; el cobro se liga a la cita (`cobros.cita_id`) y al pagarse la **confirma sola** vía webhook. El pago es opcional y a prueba de fallos: si algo sale mal, la cita queda agendada igual. | P | Agenda + Cobros | 🟢 |
| Agenda en línea + confirmación | ✅ **Confirmación**: el recordatorio (correo y WhatsApp) lleva un enlace con token; el paciente confirma o cancela con un clic, sin cuenta ni contraseña (`agenda/confirmar.php`). Una cancelación a tiempo libera el hueco. El panel marca quién confirmó ÉL MISMO (agenda y sala de espera). No se gatea: mejora el recordatorio que ya se vende en Básico y es lo que baja las faltas. ✅ **Agenda en línea**: página pública `/agenda/reservar?c=<slug>` donde el paciente agenda solo, sin registrarse; los huecos salen del horario real del médico menos bloqueos y citas tomadas (`agenda_huecos()`), y se revalidan al enviar para no agendar a dos en la misma hora. Gateado `require_modulo('agenda_online')` (Profesional y Clínica), con interruptor y enlace para compartir en Configuración. ⬜ Falta: recordatorio automático por WhatsApp (hoy el envío es manual), citas recurrentes | P/B | Agenda | 🟢 |
| Óptica | ✅ Vertical completa: graduación/receta oftálmica (OD/OI con esfera, cilindro, eje, adición, prisma, agudeza, DIP y altura; cada revisión es un registro nuevo, el historial muestra cómo avanza la vista), catálogo de micas **con precio por rango de graduación** (una mica solo se ofrece si su rango cubre la receta) y órdenes de trabajo con folio `OPT-AAAA-####`, fecha prometida calculada con los días de entrega de la mica, estados (pedido → en laboratorio → recibido → entregado), **alerta de atrasados**, anticipos y saldo, aviso por WhatsApp al estar listos, y orden imprimible con la graduación en monoespaciada. El armazón sale del inventario (descuenta stock al mandar a tallar) o lo trae el cliente (`optica/`). Gateado `require_modulo('optica')` (Profesional y Clínica). ⬜ Falta: convenios con empresas, monedero de puntos, compras/proveedores | P | Inventario | 🟢 |
...
| Ultrasonido / Imagenología | ✅ Catálogo de **protocolos por región** (qué se mide y qué se describe en cada estudio; carga inicial de 11 protocolos: abdominal, hígado y vías biliares, renal, pélvico, obstétrico de 1.er trimestre, obstétrico de 2.º–3.er, tiroides, mama, testicular, partes blandas y Doppler venoso), estudios con folio `USG-AAAA-####`, flujo de estados (programado → realizado → informado → entregado), **informe estructurado**: los renglones del protocolo se copian al estudio y se capturan como medición, texto o lista de opciones, con marca de **anormal**, más narrativa libre, impresión diagnóstica y recomendaciones. Las capturas del ecógrafo se suben en lote y se guardan en el expediente (`archivos.img_estudio_id`), por lo que aparecen solas en el portal del paciente. Informe imprimible con membrete, mediciones, impresión y las placas; aviso por WhatsApp al estar listo (`imagenologia/`). Gateado `require_modulo('imagenologia')` (plan Clínica, o add-on por consultorio desde /platform). ⬜ Falta: comparación histórica entre estudios del mismo paciente, cobro ligado a factura | C / Add-on | Expediente + Archivos | 🟢 |
| Radiología / DICOM | Subir archivos `.dcm`, **visor DICOM** en el navegador, series e instancias, comparación histórica | C / Add-on | Ultrasonido + almacenamiento grande | ⬜ |
